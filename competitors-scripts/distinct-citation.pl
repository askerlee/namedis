...
my $file = shift;
my $outfile = "${dirPrefix}distinct-citation.txt";
	...
	if($interestRecord){
		if($tagDepth == 3){
			if($el eq "author" || $el eq "editor"){
				...
				push @authorNames, replaceAuthorNames($str);
			}
			elsif($el eq "title"){
				...
				if($isPubChosen){
					if( $str =~ m{/} ){
						print $CITATIONS join("\t", $index, $pubKey, $str), "\n";
						$index++;
					}
				}
			}
...
#				print OUT "\n\n";
				...
				# only to get some statistics. it doesn't matter if a few papers are missing
