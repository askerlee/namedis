...
our @EXPORT = qw(@conceptNet @revConceptNet @terms @ICs
				...
				matchSnippet matchTitle calcMatchScore recordBestMatches addMatchScores distributeMatches
				unigramMatchTitle
...
sub iniLists
{
	$excluded = 'Actuarial science, Fictional robots, Audio engineers,
				...
				Spyware removal, Digital audio workstation software, 
...
# if $autoVivify == 1, add $term into the concept net
sub getTermID($;$)
{
...
=pod				
				my @nonstopLemmaIDs = grep { $lemmaCache[$_]->[1] != STOPWORD } @lemmaIDs;
				# if( @lemmaIDs > 1 && @nonstopLemmaIDs == 1 ), then keep stopwords
				# otherwise a multi-lemma term will become a single-lemma, and matches single lemmas
				# perfectly. Usually in this case the stopword in the term is not actually a stopword
				# e.g. "Will Interactive", a company
				# OTHERWISE, remove stopwords as follows
				if( @lemmaIDs == 1 || @nonstopLemmaIDs > 1 ){
					@lemmaIDs = @nonstopLemmaIDs;
				}
=cut
				@lemmaIDs = grep { $lemmaCache[$_]->[1] != STOPWORD } @lemmaIDs;
				
				# always remove all the stopwords in the context
...
# record  ( weight, token_indices, array_of_lemma_ids ) tuples. needs ->[0] to access weight only
sub recordBestMatches($$$$$)
{
	my ($pMaxMatchScores, $pTermIDs, $pFreqs, $tokenIndices, $winLemmaIDs) = @_;
	...
	for($i = 0; $i < @{$pTermIDs}; $i++){
		...
		if($freq > 0){
			if(! exists $pMaxMatchScores->{$id}
				...
				$pMaxMatchScores->{$id} = [ $freq, $tokenIndices, $winLemmaIDs ];
			}
		}
	}
}
...
sub matchTitle($$$$$)
{
	...
	for($i = $j = 0; $j < @words;){
		matchSnippet( [ @words[$i .. $j] ], [ @lemmaIDs[$i .. $j] ], [ @stopwordGapNums[$i .. $j] ], 
...
# calculation of $skipQueryTokenDiscount is only for comparison with $gapDiscount. Not applied anymore
				...
				if($pTermIDs && $pFreqs){
					# skip empty results
					if(@$pTermIDs){
						push @freqDeltaSets, [ \@tokenIndices, $qWinWords, \@winLemmaIDs, $pTermIDs, $pFreqs, $perfectMatchTermID ];
...
#				}
	...
	for $freqDeltaSet(@freqDeltaSets){
		( $tokenIndices, $qWinWords, $winLemmaIDs, $pTermIDs, $pFreqs, $perfectMatchTermID ) = @$freqDeltaSet;
		...
		if($isTraining){
			...
			recordBestMatches(\%maxMatchScores, $pTermIDs, $pFreqs, $tokenIndices, $winLemmaIDs);
		}
...
sub compactConceptVector($$)
{
	...
	for $conceptID(keys %$pConceptVec){
		...
		# 'tokens': an array of [ titleID, token IDs in this title ]. 
		# each array ref of token IDs belongs to one match
		# 'lemmas': an array of [ lemmas ]. 
		# each cell is the array ref of lemmas of one match
		# Same tokens may match more than 1 concept. 
	...
	for $conceptID(keys %$pConceptVec){
		...
		push @{ $conceptVec2{ $newCID }{lemmas} }, @{ $pConceptVec->{$conceptID}{lemmas} };
	...
	if($ConceptVecSimiCache{$uuid}){
		...
		if(! $venueJacThres){
			...
			if( $USE_CSLR_VERSION == 1 ){
				$venueBoost = isSameCategorical( $CAT_PRIOR, $CAT_PRIOR, $CAT_PRIOR, 0.5, $vv1, $vv2, 
										\&expandSimilarVenues, $MIN_SAME_VENUE_MN_ODDS_RATIO, 4 );
				$venueBoost2 = isSameCategorical( $CAT_PRIOR, $CAT_PRIOR, $CAT_PRIOR, 0.5, $vv1, $vv2, 
													undef, $MIN_SAME_VENUE_MN_ODDS_RATIO, 4 );
			}
			else{
				$venueBoost = isSameCategorical2( $CAT_PRIOR, $CAT_PRIOR, $CAT_PRIOR, $CSLR_VENUE_UNSEEN_REDUCTION_FRAC, $vv1, $vv2, 
										\&expandSimilarVenues, $MIN_SAME_VENUE_MN_ODDS_RATIO, 4 );
				$venueBoost2 = isSameCategorical2( $CAT_PRIOR, $CAT_PRIOR, $CAT_PRIOR, $CSLR_VENUE_UNSEEN_REDUCTION_FRAC, $vv1, $vv2, 
													undef, $MIN_SAME_VENUE_MN_ODDS_RATIO, 4 );
			}
		...
		if($useUnigram){
			...
			for $concept1(keys %$cv1){
				...
				if( exists $cv2->{$concept1} ){
					...
					my $matchDiscount;
					my $doubleDiscount = $h1->{w} * $h2->{w} * $BOOST_SHARED_TERMS_WEIGHT;
					
					# if the lemma sequence matches exactly, choose the smaller matching score
					# as $matchDiscount. otherwise, the discount is their product
					if( intersectArrayOfArray( $h1->{lemmas}, $h2->{lemmas} ) ){
						$matchDiscount = min( $h1->{w}, $h2->{w} );
						if( $matchDiscount < $doubleDiscount ){
							$matchDiscount = $doubleDiscount;
						}
					}
					else{
						$matchDiscount = $doubleDiscount;
					}
					$sharedTermSimi = max( $ICs[$concept1] - $ICOffset, 0 ) * $matchDiscount;
...
#						&&   $h1->{w} + $h2->{w} >= $SHARED_CONCEPT_LEAST_IC_WEIGHT_SUM_IN_SUM
		...
		if(keys %$cv1 == 0){
			...
			if(! $useUnigram){
				...
				$maxsimi = $maxDiffTermSimi + $sharedTermSimiSum;
...
#	my $W = 1;
	...
	for $titleID(@$titleset){
		...
		for $conceptID( keys %$conceptVector ){
			...
			# in $conceptVec1, each conceptID has 6 subkeys: 'w', 'c', 'f' (from year), 't' (to year)
			# 'tokens': an array of [ titleID, token IDs in this title ]. 
			# each array ref of token IDs belongs to one match
			# 'lemmas': an array of [ lemmas ]. 
			# each cell is the array ref of lemmas of one match
			# Same tokens may match more than 1 concept. 
			# So please check 'tokens' to avoid all of them being counted in
			$conceptVec1{$conceptID}{w} += $conceptVector->{$conceptID}->[0];
			push @{ $conceptVec1{$conceptID}{tokens} }, [ $titleSN, $conceptVector->{$conceptID}->[1] ];
			push @{ $conceptVec1{$conceptID}{lemmas} }, $conceptVector->{$conceptID}->[2];
